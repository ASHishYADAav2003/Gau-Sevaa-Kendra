          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Recent Donations</h2>
              </div>
              <p className="text-lg text-gray-600 mb-8">
                Join our community of compassionate donors making a real difference
              </p>
              <div className="space-y-4">
                {(recentDonations.length > 0 ? recentDonations : [
                  { id: 'd1', name: 'Rajesh Kumar', amount: 5000, time: '2 hours ago' },
                  { id: 'd2', name: 'Priya Sharma', amount: 2000, time: '4 hours ago' },
                ]).map((donation, index) => (
                  <div
                    key={donation.id || index}
                    className={`bg-white p-4 rounded-lg shadow flex items-center justify-between transition-all duration-500 ${
                      index === donationHighlight ? 'ring-2 ring-orange-400 scale-[1.02]' : ''
                    }`}
                  >
